...
(function(){
  ...
  async function loadConfig(){
    try{
      ...
      document.getElementById('notificationHours').value=hours.length===0?'*':hours.join(', ');
      ...
      document.querySelectorAll('[name="ENABLED_NOTIFIERS"]').forEach(cb=>{
        cb.checked=enabled.includes(cb.value);
      });

      // Telegram
      document.getElementById('tgBotToken').value=config.TELEGRAM_BOT_TOKEN||'';
      ...
      document.getElementById('barkKey').value=config.BARK_KEY||'';
      document.getElementById('barkSave').checked=config.BARK_SAVE==='true'||config.BARK_SAVE===true;

      // 更新渠道配置顯示
      toggleChannelConfigs(enabled);
    }catch(error){
      showToast('載入配置失敗：'+error.message,'error');
    }
  }

  // 動態顯示/隱藏渠道配置
  function toggleChannelConfigs(enabled){
    const map={
  ...
  document.querySelectorAll('[name="ENABLED_NOTIFIERS"]').forEach(cb=>{
    cb.addEventListener('change',()=>{
      const enabled=Array.from(document.querySelectorAll('[name="ENABLED_NOTIFIERS"]:checked')).map(el=>el.value);
      toggleChannelConfigs(enabled);
    });
  });
  ...
  form.addEventListener('submit',async(e)=>{
    ...
    try{
      ...
      data.ENABLED_NOTIFIERS=Array.from(
        document.querySelectorAll('[name="ENABLED_NOTIFIERS"]:checked')
      ).map(el=>el.value);
      ...
      // 處理通知時段
      const hoursInput=document.getElementById('notificationHours').value.trim();
      ...
      data.BARK_SAVE=document.getElementById('barkSave').checked?'true':'false';
